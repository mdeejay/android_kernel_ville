...
static struct pm8xxx_mpp_init pm8921_mpps[] __initdata = {
	...
	PM8XXX_MPP_INIT(PM8XXX_AMUX_MPP_3, D_BI_DIR, PM8901_MPP_DIG_LEVEL_MSMIO, BI_PULLUP_OPEN),
	PM8XXX_MPP_INIT(PM8XXX_AMUX_MPP_4, D_BI_DIR, PM8901_MPP_DIG_LEVEL_L5, BI_PULLUP_OPEN),
	PM8XXX_MPP_INIT(PM8XXX_AMUX_MPP_11, D_BI_DIR, PM8901_MPP_DIG_LEVEL_MSMIO, BI_PULLUP_OPEN),
	PM8XXX_MPP_INIT(PM8XXX_AMUX_MPP_12, D_BI_DIR, PM8901_MPP_DIG_LEVEL_L5, BI_PULLUP_OPEN),
...
static struct pm8xxx_mpp_init pm8921_ddc_on_mpps[] = {
	/* External 5V regulator enable; shared by HDMI and USB_OTG switches. */
	PM8XXX_MPP_INIT(PM8XXX_AMUX_MPP_3, D_BI_DIR, PM8901_MPP_DIG_LEVEL_MSMIO, BI_PULLUP_10KOHM),
	PM8XXX_MPP_INIT(PM8XXX_AMUX_MPP_4, D_BI_DIR, PM8901_MPP_DIG_LEVEL_L5, BI_PULLUP_10KOHM),
	PM8XXX_MPP_INIT(PM8XXX_AMUX_MPP_11, D_BI_DIR, PM8901_MPP_DIG_LEVEL_MSMIO, BI_PULLUP_10KOHM),
	PM8XXX_MPP_INIT(PM8XXX_AMUX_MPP_12, D_BI_DIR, PM8901_MPP_DIG_LEVEL_L5, BI_PULLUP_10KOHM),
};
static struct pm8xxx_mpp_init pm8921_ddc_off_mpps[] = {
	/* External 5V regulator enable; shared by HDMI and USB_OTG switches. */
	PM8XXX_MPP_INIT(PM8XXX_AMUX_MPP_3, D_BI_DIR, PM8901_MPP_DIG_LEVEL_MSMIO, BI_PULLUP_OPEN),
	PM8XXX_MPP_INIT(PM8XXX_AMUX_MPP_4, D_BI_DIR, PM8901_MPP_DIG_LEVEL_L5, BI_PULLUP_OPEN),
	PM8XXX_MPP_INIT(PM8XXX_AMUX_MPP_11, D_BI_DIR, PM8901_MPP_DIG_LEVEL_MSMIO, BI_PULLUP_OPEN),
...
void pm8921_ddc_mpp_on(void)
{
	int i, rc;
	for (i = 0; i < ARRAY_SIZE(pm8921_ddc_on_mpps); i++) {
		rc = pm8xxx_mpp_config(pm8921_ddc_on_mpps[i].mpp,
					&pm8921_ddc_on_mpps[i].config);
		if (rc) {
			pr_err("%s: pm8xxx_mpp_config: rc=%d %d\n", __func__, rc, i);
		}
	}
}
void pm8921_ddc_mpp_off(void)
{
	int i, rc;
	for (i = 0; i < ARRAY_SIZE(pm8921_ddc_off_mpps); i++) {
		rc = pm8xxx_mpp_config(pm8921_ddc_off_mpps[i].mpp,
					&pm8921_ddc_off_mpps[i].config);
		if (rc) {
			pr_err("%s: pm8xxx_mpp_config: rc=%d %d\n", __func__, rc, i);
		}
	}
...
uint32_t msm_hdmi_on_gpio[] = {
	GPIO_CFG(VILLE_GPIO_HDMI_DDC_CLK,  1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_16MA),
	GPIO_CFG(VILLE_GPIO_HDMI_DDC_DATA,  1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_16MA),
...
static struct htc_battery_cell htc_battery_cells[] = {
	...
	[2] = {
		...
		.capacity = 1650,
		.id = 255,
		.id_raw_min = 0,			/* should set to INTMIN */
		.id_raw_max = 1048576,	/* should set to INTMAX */
		.type = HTC_BATTERY_CELL_TYPE_NORMAL,
		.voltage_max = 4200,
		.voltage_min = 3200,
		.chg_param = &chg_batt_params[0],
